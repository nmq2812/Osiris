...
function useOrderUpdateViewModel(id: number) {
    ...
    const updateApi = useUpdateApi<OrderRequest, OrderResponse>(
        OrderConfigs.resourceUrl,
        OrderConfigs.resourceKey,
        id,
    );

    const { data: orderResponse } = useGetByIdApi<OrderResponse>(
        OrderConfigs.resourceUrl,
        OrderConfigs.resourceKey,
        id,
    );
    ...
    useEffect(() => {
        if (orderResponse) {
            ...
            setVariants(
                orderResponse.orderVariants.map(
                    (orderVariant) => orderVariant.variant,
                ),
            );
        }
    }, [orderResponse]);
    ...
    const handleFormSubmit = form.onSubmit((formValues) => {
        ...
        if (prevFormValues && !MiscUtils.isEquals(formValues, prevFormValues)) {
            ...
            const deletedOrderVariantKeyRequests: OrderVariantKeyRequest[] =
                prevFormValues.orderVariants
                    .map((orderVariantRequest) => orderVariantRequest.variantId)
                    .filter(
                        (variantId) =>
                            !formValues.orderVariants
