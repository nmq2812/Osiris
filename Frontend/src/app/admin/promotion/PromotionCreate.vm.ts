...
function usePromotionCreateViewModel() {
    ...
    const [addProductMode, setAddProductMode] = useState<AddProductMode>(
        AddProductMode.CATEGORY,
    );
    const [categories, setCategories] = useState<CategoryResponse[]>([]);
    const [products, setProducts] = useState<ProductResponse[]>([]);

    const createApi = useCreateApi<PromotionRequest, PromotionResponse>(
        PromotionConfigs.resourceUrl,
    );

    const handleFormSubmit = form.onSubmit((formValues) => {
        if (
            addProductMode === AddProductMode.CATEGORY &&
            formValues.categoryIds.length === 0
        ) {
            form.setFieldError(
                "categoryIds",
                "Cần thêm ít nhất 1 danh mục sản phẩm",
            );
        } else if (
            addProductMode === AddProductMode.PRODUCT &&
    ...
    const handleDeleteCategoryFinder = (categoryResponse: CategoryResponse) => {
        form.setFieldValue(
            "categoryIds",
            form.values.categoryIds.filter(
                (categoryId) => categoryId !== categoryResponse.id,
            ),
        );
        setCategories((categories) =>
            categories.filter(
                (category) => category.id !== categoryResponse.id,
            ),
        );
    };
