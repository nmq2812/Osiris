...
function ProductSpecificationRow({
    ...
    const handleProductSpecificationSelect = (
        specificationInfos: string | null,
        ...
        const currentSpecifications = new CollectionWrapper(
            specifications.content.map((item, index) =>
                index === specificationIndex ? specification : item,
            ),
        );
        setSpecifications(currentSpecifications);

        const currentSpecificationsIds = currentSpecifications.content.map(
            (item) => item.id,
        );
        ...
        setSpecificationSelectList(
            specificationSelectList.map((option) => {
                if (
                    ...
                    !currentSpecificationsIds.includes(
    ...
    const handleProductSpecificationValueInput = (
        specificationValue: string,
        specificationIndex: number,
    ) => {
        const currentSpecifications = new CollectionWrapper(
            produce(specifications.content, (draft) => {
    ...
    const handleDeleteProductSpecificationButton = (
        specificationIndex: number,
    ) => {
        const currentSpecifications = new CollectionWrapper(
            specifications.content.filter(
                (_, index) => index !== specificationIndex,
            ),
        );
        setSpecifications(
            currentSpecifications.totalElements !== 0
        ...
        setSpecificationSelectList(
            specificationSelectList.map((option) => {
                if (
                    option.disabled === true &&
                    JSON.parse(option.value).id ===
                        specifications.content[specificationIndex].id
                ) {
                    return { value: option.value, label: option.label };
                }
                return option;
    ...
    return (
        ...
        >
            ...
            <Select
                ...
                onChange={(specificationInfos) =>
                    handleProductSpecificationSelect(specificationInfos, index)
                }
            />
            <TextInput
                sx={{ width: "100%" }}
                ...
                onChange={(event) =>
                    handleProductSpecificationValueInput(
