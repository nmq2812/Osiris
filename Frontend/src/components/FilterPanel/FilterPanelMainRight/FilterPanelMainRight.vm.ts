...
function useFilterPanelMainRightViewModel() {
    const { filterCriteriaList, setFilterCriteriaList } = useFilterPanelStore();

    const isDisabledCreateFilterCriteriaButton =
        filterCriteriaList.length === MAX_FILTER_CRITERIA_NUMBER;

    const handleCreateFilterCriteriaButton = () => {
        if (filterCriteriaList.length < MAX_FILTER_CRITERIA_NUMBER) {
            setFilterCriteriaList((prevState) => [
                ...prevState,
                {
                    property: null,
                    type: null,
