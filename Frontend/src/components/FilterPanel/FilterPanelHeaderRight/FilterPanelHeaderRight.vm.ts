...
import { FilterPanelHeaderRightProps } from "./FilterPanelHeaderRight";
...
function useFilterPanelHeaderRightViewModel({
    ...
    const { activeFilterPanel, setActiveFilterPanel, setFilters } =
        useAppStore();
    ...
    const handleCreateFilterButton = () => {
        ...
        const filterName =
            filterNameInputRef.current?.value || "Bộ lọc " + randomNumber;
