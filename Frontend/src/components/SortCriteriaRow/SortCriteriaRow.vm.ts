...
function useSortCriteriaRowViewModel() {
    ...
    const handleSortPropertySelect = (
        ...
        const sortCriteriaPropertyList = currentSortCriteriaList.map(
            (item) => item.property,
        );
        ...
        setSortPropertySelectList(
            sortPropertySelectList.map((option) => {
                if (
                    ...
                    !sortCriteriaPropertyList.includes(option.value)
                ) {
    ...
    const handleSortOrderSelect = (
        orderValue: string | null,
        sortCriteriaIndex: number,
    ) => {
        setSortCriteriaList(
            sortCriteriaList.map((item, index) => {
                return index === sortCriteriaIndex
                    ? { ...item, order: orderValue as OrderType }
                    : item;
            }),
        );
    };
    ...
    const handleDeleteSortCriteriaButton = (sortCriteriaIndex: number) => {
        setSortCriteriaList(
            sortCriteriaList.filter((_, index) => index !== sortCriteriaIndex),
        );
        setSortPropertySelectList(
            sortPropertySelectList.map((option) => {
                if (
                    option.disabled === true &&
                    option.value ===
                        sortCriteriaList[sortCriteriaIndex].property
                ) {
                    return { ...option, disabled: false };
                }
                return option;
