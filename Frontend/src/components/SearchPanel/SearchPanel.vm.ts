...
function useSearchPanelViewModel() {
    ...
    const [prevActiveFilter, setPrevActiveFilter] = useState<Filter | null>(
        null,
    );

    const searchInputRef = useRef<HTMLInputElement | null>(null);

    const filterSelectList: SelectOption[] = filters.map((item) => ({
        value: item.id,
        label: item.name,
    }));
    ...
    const handleSearchButton = () => {
        const currentSearchToken = searchInputRef.current
            ? searchInputRef.current.value
            : "";
        if (
