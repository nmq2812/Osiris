...
function useFilterCriteriaRowViewModel() {
    ...
    const handleFilterPropertySelect = (
        propertyValue: string | null,
        filterCriteriaIndex: number,
    ) => {
        const currentFilterCriteriaList = filterCriteriaList.map(
            (item, index) => {
                const currentFilterPropertyType = propertyValue
                    ? initialFilterPropertyTypes[propertyValue]
                    : null;
                const currentFilterOperator =
                    propertyValue && currentFilterPropertyType === item.type
                        ? item.operator
                        : null;
    ...
    const handleFilterOperatorSelect = (
        ...
        const currentFilterCriteriaList = filterCriteriaList.map(
            (item, index) => {
                return index === filterCriteriaIndex
                    ? { ...item, operator: operatorValue as FilterOperator }
                    : item;
            },
        );
    ...
    const handleDeleteFilterCriteriaButton = (filterCriteriaIndex: number) => {
        setFilterCriteriaList(
            filterCriteriaList.filter(
                (_, index) => index !== filterCriteriaIndex,
            ),
        );
    };
