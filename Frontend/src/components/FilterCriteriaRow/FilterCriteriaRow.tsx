...
import { EntityPropertyType } from "@/datas/EntityProperty";
import { SelectOption } from "@/datas/SelectOption";
import FilterUtils, { FilterCriteria } from "@/utils/FilterUtils";
import useFilterCriteriaRowViewModel from "./FilterCriteriaRow.vm";
...
function FilterCriteriaRow({ filterCriteria, index }: FilterCriteriaRowProps) {
    const {
    ...
    const isSelectedFilterProperty =
        filterCriteria.property && filterCriteria.type;

    const isDisabledFilterValueInput =
        !(isSelectedFilterProperty && filterCriteria.operator) ||
        FilterUtils.filterOperatorIsNullAndIsNotNullList.includes(
    ...
    switch (filterCriteria.type) {
        case EntityPropertyType.STRING:
            filterOperatorSelectList =
                FilterUtils.filterStringOperatorSelectList;
        ...
        case EntityPropertyType.NUMBER:
            filterOperatorSelectList =
                FilterUtils.filterNumberOperatorSelectList;
            filterValueInputFragment = (
                <NumberInput
                    sx={{ width: "100%" }}
                    placeholder="Nhập giá trị"
        ...
        >
            ...
            <Select
                ...
                onChange={(propertyValue) =>
                    handleFilterPropertySelect(propertyValue, index)
                }
            />
            <Select
                sx={{ width: "100%" }}
                ...
                onChange={(operatorValue) =>
                    handleFilterOperatorSelect(operatorValue, index)
                }
